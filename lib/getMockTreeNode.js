...
function getMockTreeNode(filePath) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const nodeFilePath = path.resolve(`./file/${thriftFileName}_nodes.json`);
      ...
      if (!fs.existsSync(path.dirname(nodeFilePath))) {
        fs.mkdirSync(path.dirname(nodeFilePath));
      }
        ...
        if (line) {
          if (line === '}') {
            ...
            if (tree[editingName] && tree[editingName]._type === 'enum') {
              ...
              // console.log(services[editingName], line, line.match(/\s(\S*)\(/));
