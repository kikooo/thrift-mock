...
const input = fs.createReadStream('./thrift/hello/hello.thrift');

// // 记录上次修改时间
// const lastModifiedTime = fs.Stats.mtimeMs;
...
rl.on('line', line => {
  ...
  if (line === '}') {
    ...
    if (tree[editingName] && tree[editingName]._type === 'enum') {
      ...
      tree[editingName].value[key] = value;
...
rl.on('close', () => {
  const streamWriter = fs.createWriteStream('treeNode.json');
  streamWriter.write(JSON.stringify({ nodes: tree, services }) + os.EOL);
});
